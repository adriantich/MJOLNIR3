...
#'    sequenced across multiple lanes on a Novaseq. In this case, this function has
...
#'    In this case this function will trim the primers and separate the sequences
...
#' this function has to be run separately for each library and the option
#' \emph{multilane} must be set to TRUE.
#'
#'
...
#' Files required:
#'
#' - \emph{metadata file}. This function will read the
...
#'      If the "fastq_name_R1" column is not in the metadata, This function will use the
#'      \emph{lib_prefix}.
#'
#'
...
#' samples are already demultiplexed.
#'
#' @param cores Numeric. Number threads to run cutadapt.
#'
...
#'
#' @param original_samples Character string. Name of the column in the metadata
#' table that contains the original sample names.
#'
#' @param mjolnir_agnomens Character string. Name of the column in the metadata
#' table that contains the mjolnir agnomen names.
#'
...
demultiplex <- function(R1_filenames = "",
  ...
  colnames(metadata)[colnames(metadata) == original_samples] <- "original_samples"
  colnames(metadata)[colnames(metadata) == mjolnir_agnomens] <- "mjolnir_agnomens"
  ...
  if ('fastq_name_R1' %in% names(metadata)) {
    ...
    message("Demultiplexing the following initial FASTQ files:")
    ...
    for (i in seq_along(R1_filenames)) {
      ...
      message(paste0("Processing the ", lib, " library."))
      ngsfile <- read.csv(ngsfilter_files[grep(lib, ngsfilter_files)],
                          sep = "\t", header = FALSE)
      message(paste0("Splitting initial ", R1_file, " & ",
                     R2_file,
