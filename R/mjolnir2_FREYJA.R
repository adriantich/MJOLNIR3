...
#'
#' @param score_obialign Numeric. Minimum threshold to retain a sequence
#' in the qualiy filtering after pairalignment. If higher than 1, number of
#' nucleotides matching in the overlapping region will be considered. If
#' between 0 and 1, the fraction of matching nucleotides will be considered.
#'
...
#' R1_filenames <- c(
#'   "ULO1_R1.fastq.gz", "ULO2_R1.fastq.gz", "ULO3_R1.fastq.gz",
#'   "ULO4_R1.fastq.gz"
#' )
#'
...
#' cores <- 7
#'
#' mjolnir1_RAN(R1_filenames,
#'   lib_prefix = lib_prefixes, experiment = experiment,
...
mjolnir2_FREYJA <- function(experiment = NULL, cores = 1, Lmin = 299, Lmax = 320,
  ...
  if (score_obialign <= 1) {
    score_tag <- "score_norm"
  } else {
    score_tag <- "score"
    if (score_obialign %% 1 != 0) {
      warning("score_obialign is not an integer. It will be truncated.")
      score_obialign <- floor(score_obialign)
    }
  }
  ...
  for (i in seq_along(agnomens)) {
    ...
    filtering_commands <- c(filtering_commands, paste0(
      ...
      "obi grep -p \"sequence[\'", score_tag, "\' ] > ", score_obialign, "\" ", tmp, experiment, "_", agnomens[i], "_FREYJA/aligned_seqs ", tmp, experiment, "_", agnomens[i], "_FREYJA/good_seqs ; ",
